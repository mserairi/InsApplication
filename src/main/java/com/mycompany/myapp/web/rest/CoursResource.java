...
public class CoursResource {
    ...
    public ResponseEntity<Cours> partialUpdateCours(@PathVariable(value = "id", required = false) final Long id, @RequestBody Cours cours)
        ...
        Optional<Cours> result = coursRepository
            ...
            .map(
                existingCours -> {
                    if (cours.getNumero() != null) {
                        existingCours.setNumero(cours.getNumero());
                    }
                    if (cours.getLibille() != null) {
                        existingCours.setLibille(cours.getLibille());
                    }
                    if (cours.getDescription() != null) {
                        existingCours.setDescription(cours.getDescription());
                    }
                    if (cours.getSeuil() != null) {
                        existingCours.setSeuil(cours.getSeuil());
                    }
                    if (cours.getDuree() != null) {
                        existingCours.setDuree(cours.getDuree());
                    }
                    if (cours.getPeriode() != null) {
                        existingCours.setPeriode(cours.getPeriode());
                    }
                    if (cours.getFrequence() != null) {
                        existingCours.setFrequence(cours.getFrequence());
                    }
                    if (cours.getAgiminrec() != null) {
                        existingCours.setAgiminrec(cours.getAgiminrec());
                    }
                    if (cours.getAgemaxrec() != null) {
                        existingCours.setAgemaxrec(cours.getAgemaxrec());
                    }
